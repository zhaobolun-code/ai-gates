...
# 漂移时：
#   1) 写 .cursor/hooks-log/hooks-policy-drift.json（侧写，防 additional_context 偶发未注入）
...
try {
    ...
    $logDir = Join-Path $cursorRoot "hooks-log"
    if (-not (Test-Path $logDir)) {
        New-Item -ItemType Directory -Path $logDir -Force | Out-Null
    }
...
Side channel: .cursor/hooks-log/hooks-policy-drift.json
