...
#
# allow 旁路：
#   1. kill switch：存在 .cursor/hooks-log/pm-gate-disabled 时整条检查临时关闭（仍写审计）。
#   2. 该 conversation_id 在新鲜度窗口内有有效 [PM] 标记 → allow。
#
# .cursor/** 分级豁免（2026-08-03 自我治理轻门禁，替代原"全豁免"）：
#   Level 0 全豁免 allow：CHANGELOG.md 自身；.cursor/hooks-log/**（运行时目录：
...
# 归一化：返回 .cursor/ 之后的相对部分（统一正斜杠），供 Level 0/1 判定复用
function Get-CursorRelativePath {
    param([string]$FilePath)
    if (-not $FilePath) { return $null }
    $norm = $FilePath -replace '\\', '/'
    $m = [Regex]::Match($norm, '(?:^|/)\.cursor/(.*)$')
...
function Test-CursorLevel0Path {
    ...
    # .cursor/hooks-log/** 运行时目录（pm-gate.json / changelog-writes.json / *.log）
...
if (Test-CursorInfraPath -FilePath $filePath) {
    ...
    if (Test-CursorLevel1Path -FilePath $filePath) {
        ...
        if (-not (Test-Path -LiteralPath $changelogFile)) {
            $hint = "逃生：先写 .cursor/skills/CHANGELOG.md（CHANGELOG 自身 Level 0 豁免）后重试；或人工确认后放置 .cursor/hooks-log/pm-gate-disabled（kill switch，临时全放行）后重试；或手动编辑目标文件。"
        ...
        if (-not $changelogEntry -or -not $changelogEntry.lastChangelogWriteAtUtc) {
            ...
            $hint = "逃生：先写 .cursor/skills/CHANGELOG.md（CHANGELOG 自身 Level 0 豁免）后重试；或人工确认后放置 .cursor/hooks-log/pm-gate-disabled（kill switch）后重试；或手动编辑目标文件。"
        ...
        if ($changelogAgeMinutes -gt $FreshnessMinutes) {
            $hint = "逃生：重新写 .cursor/skills/CHANGELOG.md（Included 条目）后再试；或人工确认后放置 .cursor/hooks-log/pm-gate-disabled（kill switch）后重试；或手动编辑目标文件。"
...
if (-not (Test-Path -LiteralPath $gateFile)) {
    $hint = "逃生：先在本会话回复中发出 [PM] 标记（如「PM 判定：…」），待 mark-pm-gate 打点后重试；或人工确认后放置 .cursor/hooks-log/pm-gate-disabled（kill switch）后重试；或手动编辑目标文件。"
...
try {
    ...
    $hint = "逃生：先在本会话回复中发出 [PM] 标记，待打点后重试；或放置 .cursor/hooks-log/pm-gate-disabled（kill switch）后重试；或手动编辑目标文件。"
...
try {
    ...
    $hint = "逃生：先在本会话回复中重新发出 [PM] 标记，待打点后重试；或放置 .cursor/hooks-log/pm-gate-disabled（kill switch）后重试；或手动编辑目标文件。"
...
if ($ageMinutes -gt $FreshnessMinutes) {
    $hint = "逃生：先在本会话回复中重新发出 [PM] 标记（标记已超 $FreshnessMinutes 分钟），待打点后重试；或人工确认后放置 .cursor/hooks-log/pm-gate-disabled（kill switch）后重试；或手动编辑目标文件。"
