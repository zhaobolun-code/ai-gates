...
| 发布前仅走读/假需求验证 hook 链路（ask 权限 bug、大 payload 解析失败等真实环境故障漏检） | 机制形同虚设仍当已生效；门禁实际拦不住 | 发布闸要求**真实 Cursor 会话**真演证据（`hooks-log/` 真实 DENY/ALLOW + 打点落盘）；走读仅线索（2026-08-03 ask bug / parse 失败两连教训，见 [skill-eval-checklist.md](./skill-eval-checklist.md) §I 真演记录） |
