...
**兑现勾选**：同条核对 [retrospective-metrics.md](./retrospective-metrics.md) §每窗兑现清单 1–6；交接可写 `兑现：1✅…`（半页模板 → [window-fulfillment-halfpage.md](../templates/window-fulfillment-halfpage.md) / TEAM-GUIDE §半页备忘）。
...
| PM / 策划 | 只发一轮包；确认包教「准」；Standard/Full 默认 Auto 并写退出/后果；「准」走 §F/§G；禁口令门、禁拆轮；遵守 §H |
