...
| **L1.5 加强** | Standard 且 Mandatory Code Changes 命中 project-context 回归索引模块，**或**命中 `.cursor/lessons-learned.md` 近 6 个月内有记录的模块/文件（文件热度） | L1 同 Chat；CR **优先** Subagent 隔离（见 [isolated-review.md](./isolated-review.md)） | 方案否 / CR 隔离优先 |
...
回归索引模块之外，若 Mandatory Code Changes 涉及的文件/模块在 `.cursor/lessons-learned.md` 中**近 6 个月内有记录**（即该处曾出现真实 blocker），同样触发 L1.5——不要求正式进入回归索引表才算数，"最近真的出过问题"本身就是风险信号。`策划`/`方案审核`按模块关键词扫表时顺带判断即可，不新增独立评分脚本。命中原因记为「文件热度」，与「回归索引模块」二选一或并列写入方案审核档位说明。
