...
| 回归索引 YAML 副本 | **`.cursor/regression-index.yaml`** | **否** — 从 `regression-index.template.yaml` 复制 |
| 图谱工具启用与项目误报说明 | **`.cursor/project-context.md`** §代码审核图谱工具 | **否** |
| Express 禁入路径（高回归模块） | **`.cursor/project-context.md`** §Express 车道升级 | **否** |
...
- 含项目数据的 `.cursor/regression-index.yaml` 写进 Skill 包

## 新项目 checklist

1. 运行 `.cursor/scripts/init-project-context.ps1`（或手动复制 template → **`.cursor/project-context.md`**）
2. 填写技术栈与（可选）回归索引表
3. （可选）编辑 **`.cursor/regression-index.yaml`** 并运行 `sync-regression-index.ps1`
