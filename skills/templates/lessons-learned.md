...
> **位置**：复制本模板 → 仓库根 **`.cursor/lessons-learned.md`**。  
> **大纲**：复制 [lessons-outline.md](./lessons-outline.md) → `.cursor/lessons-outline.md`。  
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| YYYY-MM-DD | 示例模块 | 示例：判定口径与常量单位不一致 | 代码审核 | 路径 | YYYY-MM-DD | CR blocker | 单位,阈值 | 用错单位量级做阈值 | 改用同单位同源常量 | CR 反推同类混用 | PressureManager | |

类型：`方案blocker` / `CR blocker` / `运行时bug` / `自引入bug` / `Skill流程` / `成功经验`。旧行新列可空；**新行必填错因、改正**。

## 用法

- **策划**：先扫 `.cursor/lessons-outline.md` → 主表；方案写「## 错题本必读（给程序员）」
