...
> Full/L3/状态机/反模式细则 → [references/](references/)。**使用说明** → [TEAM-GUIDE.md](./TEAM-GUIDE.md)（只记「项目经理 + 需求」）
...
2. Full 强制（以下 4 条**各自独立触发**，命中任一即 Full；决策树 → [references/full-lane-decision-tree.md](references/full-lane-decision-tree.md)）
   ...
   · 状态机·持久·序列化
   · 用户说「完整流程」
   · 功能性改动 且（路径命中 project-context §Express 车道升级 或 改动涉及 project-context「运行回归索引」表中的模块）——此条单独触发即 Full，不要求同时命中以上其他条
...
3. project-context §Express 车道升级 命中，或改动涉及 `.cursor/lessons-learned.md` 近 6 个月内有记录的文件/模块（文件热度）→ 最低 Standard
4. Express 准入（全部）→ Express
...
第 2 步**未**触 Full 且 Mandatory Code Changes 命中 project-context 回归索引**模块**，或命中 `.cursor/lessons-learned.md` 近 6 个月记录（文件热度）→ L1.5（热度细则 → [plan-review-tiers.md](references/plan-review-tiers.md)）。
...
**PM 自检**：结构化判定 + **你下一步**；express-slice；A#；L1.5 派发块；**一轮确认硬律**（handoff §0）；CR/Verify；止损含热修；首段=状态；未夸大 Unity/completed。
