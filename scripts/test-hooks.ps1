...
try {
    ...
    $r = Invoke-HookScript -ScriptName "pm-gate-check.ps1" -StdinJson (@{ tool_name = "Write"; tool_input = @{ file_path = ".cursor\hooks-log\pm-gate.json" }; conversation_id = $convL1None } | ConvertTo-Json -Compress)
    ...
    $r = Invoke-HookScript -ScriptName "pm-gate-check.ps1" -StdinJson (@{ tool_name = "Write"; tool_input = @{ file_path = ".cursor\package-release.ps1" }; conversation_id = $convL1None } | ConvertTo-Json -Compress)
    ...
    $r = Invoke-HookScript -ScriptName "pm-gate-check.ps1" -StdinJson (@{ tool_name = "Write"; tool_input = @{ file_path = ".cursor\_release_staging\ai_dev_x.7z" }; conversation_id = $convL1None } | ConvertTo-Json -Compress)
    ...
    $relScript = Join-Path $repoRoot ".cursor\package-release.ps1"
    $relTestVersion = "v9.9.9"
    $relTestOut = Join-Path $repoRoot ".cursor\ai_dev_$relTestVersion.7z"
    ...
    try {
        ...
        $r = Invoke-HookScript -ScriptName "..\package-release.ps1" -StdinJson $null -ExtraArgs @{ Version = $relTestVersion; ValidateScriptPath = $stubFail }
        ...
        $r = Invoke-HookScript -ScriptName "..\package-release.ps1" -StdinJson $null -ExtraArgs @{ Version = $relTestVersion; ValidateScriptPath = $stubOk }
        ...
        $r = Invoke-HookScript -ScriptName "..\package-release.ps1" -StdinJson $null -ExtraArgs @{ Version = $relTestVersion; ValidateScriptPath = $stubFail; SkipValidate = $true }
        ...
        Remove-Item -LiteralPath (Join-Path $repoRoot ".cursor\_release_staging") -Recurse -Force -ErrorAction SilentlyContinue
    }
