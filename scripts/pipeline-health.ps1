...
#
# 定位：pm-gate.json / changelog-writes.json / hooks-log/*.log 的数据目前只用于
...
if ($JsonOutput) {
    ...
    $md += ""
    $md += "> 退出码 0=健康 / 2=有退化信号。数据源：.cursor/hooks-log/（本脚本见 scripts/pipeline-health.ps1）。"
    $mdText = $md -join "`n"
    if ($OutFile) {
