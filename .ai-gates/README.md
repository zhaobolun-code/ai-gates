...
**AI 几秒就能改出一堆坏代码。正因为如此才要 gates：你用母语提需求，判定、切片、审查、验收交给流水线——降低认知负担，不降低质量门槛。你测过，才算完。**

Vibe coding 的心流是：不纠结语法、不翻文档、不预设完整架构，像画画一样凭感觉把代码构建出来。ai-gates 在工程治理和自然交互之间找到目前最优雅的平衡点——不是用打断你来换安全，而是用低感知度的护栏让你继续向前。这是 vibe coding 从玩具走到生产可用的桥。

没有强迫你放弃 vibe，也没有让你在 vibe 里裸奔。日常只回「准」：理解、方案、开工一次确认。准了之后，标准/完整车道可以 Auto 连跑实现↔审查，硬停或待验才找你——当时黑箱加速。主对话保持项目经理，改码和审查在子窗，心流不被 diff 淹没。需要安全时翻开文档窗，箱子就变白（快捷/直通可以不建夹）。Vibe 最怕的不是写错，是写错了直到生产才发现；文件夹给了你随时把黑箱掰白的权力。

## 特有机制

别处少见、这套里已经在跑的几条。不是功能清单。

- **交付闭环**：每个需求一个文件夹；结束必须离开「进行中」（签收 / 未通过）。小改一旦已经建了文件夹，测完或没人认领同样必须离开「进行中」，不能靠「还没测」一直占着。回「准」= 理解 + 方案 + 开工一次确认。做完能收尾，不能只改状态字。
- **错题闭环**：见通用典故「一错不二犯」。测挂自动写 L0 → 修好起草 pending → 你回「准」才进主表 → 下次策划必读 → 命中就升审核档。失败变成规则，不是聊天记录。典故把已验证的做法压成一词，同一结构坑不再重讲。
- **会生长**：全世界使用者一起让技能进化。AI 每犯一个错、每留下一条验过的结构，都是进化契机——回「准」进本项目错题本或典故表；另说「上传」（且本机 GitHub 命令行已登录）才把去项目点名的条目交到已开通的公开收集仓（[ai-gates-collect](https://github.com/zhaobolun-code/ai-gates-collect)）：谁都可以开分支交 pull request，合并要有权限。收集仓合并还不等于你的包已更新。维护者抽进技能真源后，你说 `项目经理 升级 ai-gates` 才收下大家已经长好的部分。不是装完就冻住，也不是自动改规则。
...
传统 vibe coding 把认知负担卸掉之后，下面四件事会一起炸。用 ai-gates 和不用的区别：

| | 不用 ai-gates | 用 ai-gates |
| --- | --- | --- |
| **质量不可控** | 绿日志、模型自审就当修好 | 验收 A#、隔离 CR、切模型审核；你看见现象才算完 |
| **债务快速堆积** | 同一错再犯、无限小补丁 | 错题本、典故、止损链、物理口径（禁场景特化） |
| **依赖缠成乱麻** | 随手加库、抄外仓、范围无声变大 | 先读本仓、选型、外仓只抽结构、Delta 防扩围 |
| **小改引发雪崩** | 大改仍当小改硬撑 | 四车道（文件变多/跨模块立刻升道）、**热度**（挂过的模块再改加强审）、回归索引、止损 |

护栏按风险在后台升降：热度只加强已在标准上的审核，不单独改车道。当时黑：一轮确认、Auto、子窗。事后白：文档窗。你日常看见的仍是一条「你下一步」。不是用打断换安全。

## 30 秒安装，3 分钟熟练

方式 A 把安装交给一段粘贴；装好之后你不必背机制表。日常只会三步：提需求、回「准」、按提示验收。
...
| 车道 | 流程中的样子 |
| --- | --- |
| **Express**（恰好 1 个文件的注释/文案/常量） | 一句话切片 → 确认 → 改 → 一行自检；不派独立 CR。日常不建夹；一旦建了夹，结束须迁走 |
| **Direct**（默认：有行为或 2～3 个文件） | 对话内 A#/切片（不落盘）→ 确认 → 改 → 隔离 CR |
| **Standard**（>3 文件 / 跨模块 / API / 存档 / 说不清） | 先写 plan-lite 方案 → 方案审 → 确认 → 改 → 隔离 CR |
| **Full**（用户「完整流程」，或已在标准上再止损） | 执行文档 → 方案审 → 确认 → 改 → 双轮升模型审核 |

每步都要验收——A# 事先写清「怎样算通过」，以你亲眼所见为准。过程中文件变多、碰到 API/存档/跨模块立即改判车道，不硬撑；热度只加强已在标准上的审核，不单独改车道。简单/紧急小改走 Express / Direct 快通道（一句话切片、日常不落盘、无冗余步骤）——门禁按风险升降，不为小改设卡。一旦已经建了文件夹，测完或没人认领仍须迁走。
...
- **Windows：完整支持**。机器强制 hooks（PM 写门禁 / 高危 git deny / Unity 编译提示，全部 `.ps1`）在 Codex CLI 0.146/0.147 与 Claude Code（2026-08-10 全链路真机确证）已实测 deny 拦截；一键安装见「30 秒安装，3 分钟熟练」。已知缺口：Codex 桌面应用对 `apply_patch` 钩子可能不触发（信任已批准仍零打点）——关键写操作后请自查 `.ai-gates/hooks-log/`。
...
机制表是**护栏说明**，不是开工前必背清单；门禁由助手执行。被拦了？deny 提示自带逃生路径；完整速查见「30 秒安装，3 分钟熟练」里的 USER-GUIDE 链接。
