...
| 车道 | 流程中的样子 |
| --- | --- |
| **Express**（一行注释/文案/常量） | 一句话切片 → 确认 → 改 → 一行自检；不派独立 CR。日常不建夹；一旦建了夹，结束须迁走 |
...
每步都要验收——A# 事先写清「怎样算通过」，以你亲眼所见为准。过程中命中升级（改动超范围、碰热度、跨模块）立即改判车道，不硬撑。简单/紧急小改走 Express / Direct 快通道（一句话切片、日常不落盘、无冗余步骤）——门禁按风险升降，不为小改设卡。一旦已经建了文件夹，测完或没人认领仍须迁走。
