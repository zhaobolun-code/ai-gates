# post-write-gate.ps1 -- Claude Code 版（合并入口）
# PostToolUse hook (matcher: ^(Write|Edit|MultiEdit|NotebookEdit)$) -- 把同一事件的两个
# 观测门合并为一次进程调用：
#   mark-changelog-write.ps1（CHANGELOG 写打点，副作用落盘）+ check-unity-compile.ps1
#   （写后 Unity 编译错误提示，可注入 additionalContext）。
#
# 语义与分开挂载时完全一致（2026-08-10 自 Codex 版 post-apply-patch-gate.ps1 复制改写）：
...
if ([string]::IsNullOrWhiteSpace($final)) {
    $final = '{"hookSpecificOutput":{"hook_event_name":"PostToolUse"}}'
}
Write-Output $final
exit 0
