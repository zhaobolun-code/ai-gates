# pre-apply-patch-gate.ps1 -- Codex 版（合并入口）
# PreToolUse hook (matcher: ^apply_patch$) -- 把同一事件的两个门禁合并为一次进程调用：
...
#   - 全部 allow → 输出 pm-gate-check 的 allow JSON（无 permissionDecision = allow）。
