# pre-bash-gate.ps1 -- Codex 版（合并入口）
...
#   - 全部 allow → 输出最后一个 allow JSON（无 permissionDecision = allow）。
#   - stdin 由本入口预读一次缓存到全局，两个子脚本共享同一 payload
#     （Read-HookStdin 全局缓存，见 codex-hooks-common.ps1）。
# 收益：apply_patch/Bash 事件的进程 spawn 从 2 次降到 1 次（长会话省时间）。
...
if ([string]::IsNullOrWhiteSpace($final)) {
    $final = '{"hookSpecificOutput":{"hookEventName":"PreToolUse"}}'
}
Write-Output $final
exit 0
