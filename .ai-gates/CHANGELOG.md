...
- 2026-09-03：迷雾大地图：同列先横后竖接到右侧沟；左边无跨列关联不画。不 bump VERSION
- 2026-09-03：用户点名给 `.ai-gates/Doc/AI流水线` 出主题旁迷雾地图。不 bump VERSION
- 2026-09-03：迷雾大地图：悬停 `classList.toggle` 的 force 必须是布尔值；缺键时 `&&` 得到 undefined 会把无关竖脊翻转点亮。不 bump VERSION
- 2026-09-02：迷雾大地图：去掉大地图悬浮框；悬停线与发亮图块对齐（两端都亮才亮横线，再接竖段）。不 bump VERSION
- 2026-09-02：迷雾小地图点选后，虚线连着的邻格按 50% 透明（不再只认 parts 含 rel，避免关联格掉进 20% 全黑）。不 bump VERSION
- 2026-09-01：README「会生长」改成：做成的办法回写典故，再抽象上传。不 bump VERSION
- 2026-09-01：README 灰箱改成白话产品介绍（为什么要有、为什么好用）；打开收到一句；USER-GUIDE 操作表对齐。不 bump VERSION
- 2026-09-01：迷雾大地图不再画「雾」块（避免压总线）；未建夹改到可开关小地图；大陆/小岛仅 N>0 显示「雾:N」并带青色内发光；悬停浮层压在小地图之上。不 bump VERSION
- 2026-08-28：通用 Skill 抹本仓路径：文档根/图谱/神类举例改读 project-context；本仓覆盖仍在 project-context。不 bump VERSION
- 2026-08-28：设计模式表抹本仓路径：词条只留症状/结构；本仓验证挪到 `.ai-gates/design-patterns.project.md`。不 bump VERSION
- 2026-08-28：README 英文文档收敛：`.ai-gates/README.en.md` 移除，英文文档唯一源=发布仓根 README（GitHub 主页，中英同步口径）；中文 README 无互链。不 bump VERSION
- 2026-08-28：迷雾地图草案：HTML 游戏大地图挂灰箱；关联度默认权重；搜索/叠钉预览/路悬停/孤岛/推荐下一站进 5.0。不 bump VERSION
...
### Included changes — 2026-09-03（README 差异化 + 出图门 + 编排整治 · 不 bump）

- **README 对齐现行 skill 并补差异化定位**：新增「和同类技能包比，差别在哪」（跨平台一份真源 / 三档证据防伪绿 / 四车道自动选 / 两本账跨项目自进化 / 规则点名不灌库 / 迷雾地图）；Vibe coding 补「一个入口」「护栏不只靠自觉」「不测不算通过」；灰箱写明对人/对助手的帮助；安装段补方式 B/C（一条命令 / 手动下载）+ 检查健康/升级；平台段写明 pwsh 前提。修正两处被外部摘要暴露的缺陷：「典故」术语化与错题本成对命名（两本账）；删除无度量出处的「少用 70% 模型用量」定量宣称，改为机制表述。中英同步（英文 README 全量重写对齐）。
- **出图门细化**：文档状态改变后——无图则判断能否成图并问「是否创建迷雾地图」，有图则问「是否更新迷雾地图」（generate-fog-map.ps1 / fog-map-structure.md / README / USER-GUIDE）。
- **METHODOLOGY 编排整治**（内容不改主张、不动数据）：多列宽表转纵排短列表（消除单行 400–1132 字符 GitHub 横向溢出）；新增「下面五节的共同口径」集中讲共有口径；删三重复述（摘要 5 段压缩、各节尾句、§11 五条重复脱敏声明）。
- **修复**：generate-fog-map.ps1 / check-pipeline-doc.ps1 / migrate-pipeline-window.ps1 / package-release.ps1 UTF-8 BOM 丢失（外部会话重存导致，validate A7 漂移 + 打包 A3 解析失败根因）；package-release.ps1 删除已移除的 README.en.md 打包残留行。

## [5.0.0] - 2026-09-01（迷雾地图 5.0 上线 · major · 发布）

### Breaking

- **改文档窗读法**：须读迷雾卡片 + 一度边；未读邻边不得自称已按 5.0.0 合规。不再以 4.x「命中才读」为现行读法（Express 机械改 / 纯代码热修不碰文档窗 / 闲聊仍可不读）。
- **默认人看图落点**：无 `-OutFile` 时 HTML 默认 `.ai-gates/verify/`（`fog-map.html` + 瘦 JSON）。

### Changed

- **岗文与结构面**：四处 checklist + `fog-map-structure.md` 正式页；CR +1 迷雾子弹。
- **生成器项目级扫描**：无参扫文档根父级 + `.ai-gates/Doc`；near 悬停差几分；覆盖度/热度分开展示。
- **灰箱**：打开迷雾地图（README + USER-GUIDE，verify HTML）；**更新**须项目经理提示、你同意后再生成；禁止有夹自动出图。草案收口结构面。
- **人看热修**：两枚徽章上大陆/组卡片可见；大地图短悬停不挡钉子正文；雾不压总线；状态五色拉对比；sat 加宽。
- **出图门与直通账本**：同一主题根能形成大地图才提示用户生成；sidecar 扫 `直通文档/{模块}/DIRECTLOG.md` 挂钉不占格。
- **助手读卡（4.x 上游）**：瘦 `fog-map.json`（无 pin 正文、无布局坐标）；入口曾为命中才读——5.0 现行读法见上 Breaking。
- **大地图沟内总线**：全局去重；竖干按接点切开，悬停只点亮真正连到该块的段。
