...
1.58 **代码认知地图**：project-context 有「热路径认知卡片 / 代码认知卡片」节且本步改其受管/observe 文件 → **先整节装载**（压缩后同样；禁止用摘要冒充）。然后：① 用触及对象的 R 打开邻居**源码**；② CodeGraph 补结构边；③ 只读当前文件。禁止 `codegraph_explore` 冒充已理解系统。改了受管文件：同 Step **优先改对应卡**，否则交接写「卡片仍准」+ 为何未改。无该节不强制。点名 [code-cognition-map.md](../references/code-cognition-map.md)
2. Read `.cursor/project-context.md`（若存在）+ 模块 README **当前风险短段**（勿整份版本史）+ **真实代码**（顺序：有认知卡片则已按 1.58 装载全图；再 **优先反复 `codegraph_explore`**；禁止因 soft budget 弃用；禁止全目录扫读）
2.1 **复用四问**（改码前，见 [execution-discipline.md](../references/execution-discipline.md)）：核对方案短表；方案漏则自补并报 PM。优先接已有 helper/Service/分支；禁止复制粘贴第二套同类逻辑。短表写「能删」则本 Step diff 的 Delta REMOVED 须点名对应符号；生产 caller 仍通断 → 不得自称本 Step 完成。命中 project-context **神类止血/补强三口** 时：落点进 Service、守净增阈/方法预算，交 CR 前写**瘦身一拍**一句。窗目标删 X 且短表写本步无可删 → 不得自称完成，停报 PM
2.11 **修 bug 根因先于保底**（Direct 同样；Express 机械改除外）：改码前自检写**根因一句**（坏在哪条因果/符号）。写不出 → 停、只读、交 PM，禁止先打补丁。禁止用夹紧、默认值、空 catch、再开一条路、force/suppress 藏症状且不删错因。交 CR 须能指出根因落点。根因在共享函数时：先核对所有 caller，一处修；禁止只在工单点名的那条调用链打补丁。连败禁止换皮再保底，走黑板/止损（[diagnosis-gates.md](../references/diagnosis-gates.md)）。允许根因分两刀。
...
6.06 **本条消息验证**：交 CR 前：适用才跑 `dotnet build` 或本步点名测试；不适用写 `not run` + 原因。Agent 自称 `locally-validated` 时本条消息必须含命令 + 退出码/失败计数。自称 **DONE** 且本步有可跑命令（含本窗点名的 `.ai-gates/scripts/*.ps1`）→ 同条必须命令+退出码/失败计数；否则必须 `DONE_WITH_CONCERNS`（不得写 DONE）。Express 未改 cs → `not run`，不强制每条 Express 都 `dotnet build`。Skill/Doc-only 仍用静态核对；不伪造 Unity Play。细则 → [evidence-levels.md](../references/evidence-levels.md)。
7. Unity 未跑写 `not run`；不得夸大证据等级；**仅业务 C#** 同一 Step/切片连续 ≥2 次交审/交自检整轮 `static-checked` 修复仍无 Unity 验证 → 停并提示先测，不再叠加。Skill/Doc-only 若 Mandatory 点名可跑脚本，同一 Step 连续 ≥2 次交审仅 `static-checked` 且无同条命令+退出码 → 停；**KEEP** 现句「微循环中间自检、Skill/纯文档假需求均不计」中的假需求不计（无脚本点名仍不计）。禁止把 Unity Play 改成 hook 闸。微循环中间自检、Skill/纯文档假需求均不计。用户测失败后交 PM 走 [diagnosis-gates.md](../references/diagnosis-gates.md) §0，禁止自行开 Step N+1；若交接已有 `Auto采纳推荐` 且 `auto_follow: yes`，按该项执行，勿再等人「准」（硬停除外）。**证据黑板**：交审级修复/测挂后再改前 Read `证据/_repair-blackboard.md` 最近 ≤3 条，禁换皮重试「禁止再做」；本刀失败同条追加一条（模板 [repair-blackboard.md](../templates/repair-blackboard.md)）。止损或 `max_repair_rounds` 触顶 → 不得再同 A# 改码，交 PM 走 §0.7 A#/口径复议。**准全自动**：**自动**在方案夹 `未完成.md`→`## 错题 L0 草稿` 记 L0（禁写主表）。
...
8.5 **经验/错题（准全自动）**：失败输出可先跑 `draft-lesson-pending.ps1` 填模板（仍须「准」才 `-Apply`，禁静默主表）。①失败：**自动** L0；②成功/根因验证后交主窗 PM **自动**落 `证据/_lesson-pending.md`（勿静默写主表）；③用户「准」后 `commit-lesson-pending.ps1 -Apply`（见 [lessons-learned.md](../references/lessons-learned.md)）。禁空话 pending。**格式强制**：教训草稿 `_lesson-pending.md` 必须按模板 [lesson-pending.md](../templates/lesson-pending.md) 用 `yaml` fenced block 撰写——首块必填 `status/date/module/lesson/source/doc/type` + 非空 `cause/fix`；可选 `keywords/prevent/scope/l0_section/outline_bucket`；格式契约以 `commit-lesson-pending.ps1` 函数体为准（`$required` 数组 + cause/fix 非空校验）；交草稿前自检 yaml 块存在——散文/标题格式会被 commit 脚本拒绝（2026-08-13 实测）。
