...
> 下列路径是核心回归**提示**，**不封车道**。PM 默认 Direct。恰好 1 个文件且机械 → Express；有行为或 2～3 个文件、无 API/存档/跨模块 → Direct；>3 或 API/存档/跨模块 → Standard。已判 Standard 且命中本表/回归索引 → 方案审 L1.5（加强审，不回头改车道）。高风险路径写在此；无则删除本节。
