...
**条款可证伪（思考碰撞回灌 · 与方案审 3.55 同句）**：下列任一条 → 方案审 **major**：

1. 成功句有机器源，失败句只禁某种做法、抓不到「自报 / 不读该源 / 缺字段」。
2. 机器检查只写「计数字段/脚本绿」未点名要比对的字段或行键（可哑字段或只比条数永远绿）。
3. Mandatory 入口接线（某 SKILL +1 行等）未写入对应 A#。
4. 手工红线与脚本证据不同句，脚本绿可单独冒充该红线已过。
