...
「准」不含抽离；「准, 抽离」/「准, 另开抽离窗」为同一确认包后缀，不是第二轮确认。
...
**用户本轮（默认教）**：`准` / `按推荐`（「准」不含抽离；「准」=定版+改码仍对）
**抽离另口令（同一确认包后缀，不是第二轮确认）**：`准, 抽离` / `准, 另开抽离窗`
...
**提示换窗（≠ 执行交接）**：本会话用户发言 ≥20 轮（不含助手/子窗/工具结果）→ 主窗 PM 在「你下一步」附一句建议回「换窗」（每窗最多 1 次；「先不换」后不再提示）。**禁止**因 ≥20 自动吐八段 / 新窗第一句；触顶/压缩也不另开第二套提示阈值。提示条件只认 20 轮，无「2 轮实现+审核」。  
**触发**（须按八段模板输出或落盘 `证据/_handover.md`，**并**附加「新窗第一句」fenced `text` 复制块）：① Auto `reason=max_auto_steps` ② 方案结案交接 ③ 用户要「交接」/「换 Chat」/「handover」/「换窗」/「卡了」/「卡了换窗」。  
**八段钉死**：当前状态 / 已做 / 下一步 / 禁区 / 依赖 / 风险 / 证据路径 / 下一口令。  
**新窗第一句**（附加、不替代八段）：任务 / 已落地 / 未完成 / 禁区 / 先读 / 旧窗检索 / 请直接做；块首指令 +「本窗无法自动新建对话」事实句；空表见 [templates/session-handover.md](../templates/session-handover.md)。  
**禁止**：灌归档全文 / 旧对话全文 / `已完成/**` 全文 / 长 Console 进八段或新窗第一句；用交接旁路「准」或待测闸。
