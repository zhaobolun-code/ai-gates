...
| **manual** | 脚本失败，Agent 已 **Write** 追加 JSONL 一行到 `.ai-gates/pipeline-snapshot.log` |
...
- 日志：`.ai-gates/pipeline-snapshot.log`（JSONL，本地可选；**勿提交**）
- TL 汇总（可选）：`.cursor/scripts/summarize-pipeline-metrics.ps1`
- **效果轻量版**（签收/结案一行）：`.ai-gates/pipeline-outcome.log` ← `append-pipeline-outcome.ps1`；汇总 `summarize-pipeline-outcome.ps1`（见 [retrospective-metrics.md](./retrospective-metrics.md) §效果轻量版）
