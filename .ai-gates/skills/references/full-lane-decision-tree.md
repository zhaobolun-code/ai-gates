...
开始：有代码改动需求
  ...
  │     或 回归索引模块？ ──是──→ 规模较大（>3 文件 / 净增删 >~150 行 / 跨模块·API·持久）→ Full；
  │                             小规模功能改动 / 仅日志注释 → Standard + L1.5
  │
...
| 功能性 + 回归核心模块且规模较大（>3 文件 / 净增删 >~150 行 / 跨模块·API·持久） | 改核心业务传递逻辑、影响多处联动的状态字段 |
...
| 核心/回归模块内小规模功能性改动（≤3 文件、无 API/持久/跨模块） | **Standard + L1.5**（不强制 Full） |
