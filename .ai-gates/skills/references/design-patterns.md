...
| Policy 策略 | 门闸 if/switch 分支膨胀 | **适用**：拓扑/门态判定可枚举化。**结构**：`*Policy`/`Evaluate*` 返回枚举态。无本仓验证 → 不采用（项目典故，不是可移植人类模式）。禁止方案要求空白仓发明 `PressureRuntimeState` / `*Policy` / `*Service`。**本仓验证**：`确定[有代码证据]` `BalanceGroupGatePolicy.cs`（`Evaluate`/`ResolveTopology*`）；`确定[有代码证据]` `PressureController.ValveAndTransferCore.cs:86` `EvaluateOneWayValveFsm`；`推断[有间接证据]` R30 执行中窗读码复核引用上述符号。 | `static-checked(静态核对)` | 单场景 dead branch；Policy 内 mutation |
