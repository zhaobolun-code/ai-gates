...
> 精简三行：每条 = **门禁** / **模型常找的借口** / **识别信号**。命中 → 按对应硬门禁处理，不因「紧急 / 已自检 / 主窗代行」豁免。评测 → `skill-eval-checklist.md` §K。

| 门禁 | 模型常找的借口 | 识别信号 |
| --- | --- | --- |
| **无本轮 `[PM]` 仍写交付物**（硬门禁 #7 / pm-gate） | 「用户已叫程序员」「只改文档不算」「上轮 PM 还新鲜」 | 本会话 120 分钟内无 Stop 打点的 `[PM]`；hooks deny `no fresh [PM]`；write-audit 无同会话 PM |
| **日志关键词冒充 A# 通过** | 「Console 已命中」「verify.json ok=true 即业务过」「黄金绿=手测签收」 | 证据等级标 `runtime-validated` 但无用户亲测句；A# 失败判据未对现象；把 Keyword/trx/黄金绿写成业务通过 |
| **伪称隔离 CR / 扩白名单自审** | 「实际已派子窗」「主窗代行也算隔离」「证据夹整夹给审」 | write-audit 单会话写完评分/CR 结论；pm-gate 无子代理；派发未点名单个 `_…派发.md` 或白名单含整个 `证据/`；评分表预写未发生的「结论」 |
...
| 把带问号的确认疑问当成开工口令并改文件 | 未拍板就落盘 | 问号句先答该不该与为什么；等确定改法或「准」再改文件。禁止把商量当「准」 |
| 把开工「准」写成开 PR/建 issue，或把技能包真源仓 issue 当收集出口 | 错通道、一准两用 | 收集仓上传走 pull request；issue 仅草稿。开工「准」只定版和改码。独立口令「上传」且本机探测成功才允许开 PR。未探测成功禁止自称已通 |
| 把收集仓合并写成已下发，或把下发写成拉收集仓 | 各项目收不到真源包、口径撒谎 | 下发只走「项目经理 升级 ai-gates」拉技能包真源仓；收集仓合并只算审查。入通用格须另「准」。未出现新 tag 不得自称已下发 |
