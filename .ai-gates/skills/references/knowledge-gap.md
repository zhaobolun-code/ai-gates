...
`证据/_knowledge-gap.md`：**每窗一个**（窗名同执行文档夹），doc-windowing 合法读写文件（同 `_lesson-pending.md` 先例，见 [lessons-learned.md](./lessons-learned.md) §L0/L1/pending）。
...
- 同族问题**跨窗**重入队 ≥2 次（不同执行窗出现同族问题即计数）→ 走机制 A 活跃度判定升级为正式条目/文档（指路 [evolution-01-self-evolution.md](../../Doc/evolution/evolution-01-self-evolution.md) 机制 A，不重复实现）。
