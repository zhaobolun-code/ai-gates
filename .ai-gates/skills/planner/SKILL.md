...
1.1 **复用四问**（写 Mandatory 前强制，见 [execution-discipline.md](../references/execution-discipline.md)）：已有吗→能复用吗→能少写/不写吗→能删吗；每 Step 落 ≤6 行短表（可并入选型短表）；未检索不得写新路径。第 4 问最低证据须三选一：① Delta Spec **REMOVED** 点名本步要删的符号；或 ② 写「本步无可删」且改动路径上已点名旧轨各有不能删原因（谁还调用 / 口径要留 / **什么现象才改**；缺升级触发 = 填空；路径上无已点名旧轨则可只写「本步无可删」）；或 ③ 写了「能删」则 Delta REMOVED 必须点名对应符号——生产 caller 仍通断 = 第 4 问未完成。禁止「能删 X；Y 不删」无原因即满足。神类只增不减句不得替代三选一。若 project-context 有 **神类止血/补强三口**：Mandatory 须用**替换句式**，超净增阈/方法预算须 Service·拆分·REMOVED 或豁免句。窗目标删 X 时第 4 问按叠加写，禁止用②完成
...
1.16 **代码认知地图**（有 project-context「热路径认知卡片 / 代码认知卡片」节时）：写该节受管类型的 Mandatory 前 **整节装载一次**；用 R 决定还要打开的邻居源码。禁止只读一张卡冒充已理解系统；禁止用 CodeGraph 冒充全图。本 Step 改受管文件时 Mandatory 须写「卡片仍准」或「已改卡片：{对象}」（与 developer 1.58 / CR 1.18 同判）。无该节不强制。点名 [code-cognition-map.md](../references/code-cognition-map.md)
