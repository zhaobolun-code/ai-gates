...
1.14 **无命令自称 locally-validated（硬挡）**：Agent 无命令输出却自称 `locally-validated` → **major**（写死 **major**，不是 blocker，不是 nit）。Skill/Doc 点名脚本时，无同条命令自称 locally-validated 仍 **major**。用户口头签收不算这条 major。禁止把用户口头改写成 Agent 已 Play。细则 → [evidence-levels.md](../references/evidence-levels.md)。
...
1.146 **DONE 无同条命令（硬挡）**：自称 **DONE** 且本步有可跑命令却无同条退出码/失败计数 → **major**；PM 见 DONE 无命令 → 不得当无事派 CR。Skill/Doc 点名脚本时，无同条命令自称 DONE / locally-validated 仍 **major**；PM 见 DONE 无命令不得派 CR。KEEP 隔离 CR 由 PM 派。不新造证据等级。
...
1.18 **代码认知地图**：project-context 有认知卡片节时，未装载该节却自称已理解热路径、或用 CodeGraph/搜索冒充全图 → **major**。改了受管文件但 Mandatory/交接无「卡片仍准」或「已改卡片：{对象}」→ **major**（与 project-context 过期检查同判）。无该节不强制。点名 [code-cognition-map.md](../references/code-cognition-map.md)
...
1.5 **经验/错题（准全自动）**（见 [lessons-learned.md](../references/lessons-learned.md)）：失败输出起草用 `draft-lesson-pending.ps1`（须「准」才 `-Apply`，禁静默主表）；blocker 修复确认后交主窗 PM **自动**起草 `证据/_lesson-pending.md`（类型默认 `CR blocker`）；成功路径同条可代拟 pending；**须用户「准」**才写入主表；禁静默/空泛；扫表命中须更新「最近命中」。
...
- **质量维（默认）**：只实现当前 Step/未扩 scope；覆盖且未越出 A#（[acceptance-and-delta.md](../references/acceptance-and-delta.md)）；未点「准, 抽离」却整段搬家/新抽象 → **规格轴**越出 A#（不得只记规范轴投机抽象）；README/project-context 硬约束（含 **神类止血/补强三口** 若有）；边界/状态安全；语义理解错误（名实不符→查证，改行为记 major/blocker）；**复用四问**（[execution-discipline.md](../references/execution-discipline.md)）：已有 Unity/.NET/平台 API 可调用却新包一层→nit；因此形成并行实现或违背物理口径→major；可复用却复制新实现→major；只增不删且无说明→major；形成并行实现无互斥句→blocker；改码步窗目标点名要删的符号、diff 生产 caller 仍通断、Mandatory 未点名下一刀 REMOVED → 规格轴 **major**；Discover-only 无 diff → N/A；过度设计/死代码/类膨胀→major；新方法体超预算无豁免→major；缺瘦身一拍且膨胀明显→nit/major；禁把 static-checked 写成 runtime-validated；**根因先于保底**：修 bug 的 diff 只在出口夹紧/默认/空 catch/加第二条路径、未改或未删错因落点 → **major**；自检无根因一句 → **major**；保底与错因并存当修好 → **major**；连败换皮保底 → 黑板禁项 **major/blocker**；**设计模式结构核对**（[design-patterns.md](../references/design-patterns.md)）：Mandatory 声明的模式须与词条「结构/禁用边界」一致；明显可 inline 却新抽象（模式崇拜）→ **major**；与 §典故 重复造词 → **major**；**证据黑板**（派发点名时可 Read）：手法与最近条「禁止再做」实质相同→**major/blocker**；`repair_rounds≥1` 却无黑板/未注入→**major**；止损或 `max_repair_rounds` 触顶后仍同 A# 交修→**blocker**；**错题必读**：方案已点名错因/改正行，diff 仍复现同错因→**major**；**外仓对照**：方案启用后 diff 出现外仓路径/类名或整文件粘贴→**blocker**（[external-compare.md](../references/external-compare.md)）
