...
`升级 ai-gates`（可带 GitHub 地址）= **联网更新**：先比对官方源最新 tag 与本地版本（install-info 优先，否则读 `skills/VERSION`），有新版才下载替换本地库（项目状态文件保留），随后跑 link-platform 校验/补齐传送门；网络不可用或无新版时回退「本地已解压包重接传送门」。替换前 PM 白话说明来源与版本（口令本身即授权）。
