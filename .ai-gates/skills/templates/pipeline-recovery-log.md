...
> **位置**：复制本模板 → 仓库根 **`.ai-gates/pipeline-recovery-log.md`**（勿提交敏感信息时可加入 `.gitignore`）。
> **何时追加**：用户发送 `按 CORE 重来` / `流水线重来` 时，Agent 在恢复流程末尾追加一行。
> **快照日志**（全量 PM 轨迹）：`.ai-gates/pipeline-snapshot.log` — 见 [pipeline-snapshot-log.md](./pipeline-snapshot-log.md)
