...
- **复用四问**（写 Mandatory 前；见 [execution-discipline.md](../references/execution-discipline.md)）：
  ...
  - 能删：[三选一：① REMOVED 点名本步要删的符号；或 ②「本步无可删」且改动路径上已点名旧轨各有不能删原因（谁还调用 / 口径要留；路径上无已点名旧轨则可只写「本步无可删」）；或 ③ 写了「能删」则 Delta REMOVED 必须点名对应符号——生产 caller 仍通断 = 第 4 问未完成。禁止「能删 X；Y 不删」无原因即满足。窗目标删 X 时不得用②当完成态；Discover-only 须点名下一刀 REMOVED+替代边（或缺口）]
