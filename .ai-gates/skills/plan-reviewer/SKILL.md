...
3.10 **复用四问**（[execution-discipline.md](../references/execution-discipline.md)）：每 Step 缺「已有/复用/少写或不写/能删」短表 → **blocker**；明显可复用既有 helper/Service/门闸却新开并行实现或新抽象 → **blocker**；REMOVED 恒为「无」且 Mandatory 只往神类堆逻辑、无抽离说明 → **major**。四问表已有但第 4 问填空 → **blocker** 不得定版。填空=只写「不删」/KEEP/「本步无可删」且改动路径上已点名旧轨未各写不能删原因；或写了「能删」但 Delta Spec REMOVED 未点名对应符号
