...
1.15 **图谱（审核岗）**：Mandatory 点名业务源码时，**优先 CRG**（`detect-changes` / impact / review context；业务在子模块则 `--repo` 点子模块根，见 [codegraph-probe.md](../references/codegraph-probe.md) 与 project-context）；需核对符号语义/原文时再窄用 CodeGraph。禁止两套完整双跑；无图谱按 codegraph-probe 降级（不得伪造「已核对调用链」）。结构图 **不得**冒充已装载认知全图（[code-cognition-map.md](../references/code-cognition-map.md)）
1.16 **代码认知地图（轻量）**：project-context 有认知卡片节且 Mandatory 点名受管文件时，方案须能对上 index/observe/exclude；把迷雾「一张卡」写成热路径读法、或用搜索冒充全图 → **major**。无该节不强制。
...
3.10 **复用四问**（[execution-discipline.md](../references/execution-discipline.md)）：每 Step 缺「已有/复用/少写或不写/能删」短表 → **blocker**；明显可复用既有 helper/Service/门闸却新开并行实现或新抽象 → **blocker**；REMOVED 恒为「无」且 Mandatory 只往神类堆逻辑、无抽离说明 → **major**。四问表已有但第 4 问填空 → **blocker** 不得定版。填空=只写「不删」/KEEP/「本步无可删」且改动路径上已点名旧轨未各写不能删原因；或写了不能删原因但无升级触发（什么现象才改 / 谁还调用）；或写了「能删」但 Delta Spec REMOVED 未点名对应符号。窗目标删 X 且走②（或等价「本步无可删」）且未点名下一刀 REMOVED → **blocker**；同一升级触发连续 ≥2 Step 未触发且无新钉事实 → 第 4 问填空 **blocker**
3.10.1 **确认包【重构候选】**：确认包将发出时缺【重构候选】栏且本应有候选 → **major/blocker**。一行「无」或 Express/无症状/热修整栏省略 **不是** 缺栏。禁止把「无」当缺栏 blocker。
...
3.12 **设计模式外部校验（轻量 · evolution-03 机制 C）**：Step Mandatory 声明采用 [design-patterns.md](../references/design-patterns.md) 词条时，缺**触发症状**描述 → **blocker**；缺**强制选型句**（症状+理由+出处）→ **major**；词条与 shared-language §典故 **同义双挂**（如又登记神类止血）→ **blocker**。按词条类型分：对「人类/经典模式」行（事件分发/命令/能力模板/简单工厂/特效适配器），症状命中却因无锚点写不采用 → **major**；无症状仍推模式 → 仍 blocker/major（缺症状写模式名=blocker）。对 State 快照 / Policy 策略 / Seam Service：无验证实例仍推 → 仍 **major**。无采用声明：须有字面 `本步不采用 design-patterns 词条`；缺句 → **major**。需求源未写模式名不得因此 major。
...
3.15 **晋升闸文案（evolution-03-promote）**：晋升方案写「三重闸」却列四条 → **major**；本机未探测仍写已登录、或空口「已跨项目」而无收集仓同词证据 → **blocker**（[design-patterns.md](../references/design-patterns.md) §晋升闸）。收集仓已开通后跨项目 ≥2 **可实施**，不得再以「gh 未接线」挡可跑。
...
3.18 **本地自进化环（轻量）**：把收集仓已开通写成技能包已下发、或本机未探测仍写已登录 → **blocker**；未「准」写入 `shared-language.md` §典故或 `anti-patterns.md` → **blocker**；写成第七岗 / 平行队列 / 新建通用错题主表 → **blocker**。
...
3.23 **根因先于保底**：本步修可见故障却无根因一句 → **major**；Mandatory 以夹紧/默认值/空 catch/第二条路径为修复、选型无「不选保底」或未写删哪条错因 → **major**；无根因假设却定版保底实现 → **blocker**。不要求一次找到终极根因。连败仍走 3.8 止损，不另开闸。
