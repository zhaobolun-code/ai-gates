...
- **当前版本值**：[VERSION](./VERSION)（唯一来源，现以该文件为准）；发布状态与历史见 [CHANGELOG.md](../CHANGELOG.md) **顶行「当前 LTS」**
- **禁止**把下面「v3.1.4 定版」段当成当前 VERSION。v3.1.4 是 2026-07 历史定版。
...
v3.1.3 观察期已于 **2026-07-16** 由 TL「bump / 转正」关闭；**v3.1.4 定版**为 2026-07 历史（勾选状态保留）。当前 LTS 只认 [VERSION](./VERSION) + [CHANGELOG.md](../CHANGELOG.md) 顶行。历史勾选状态：
...
| **用户可见** | 回复须含字面 `[PM]` YAML（给机器，不讲解字段）；Express 只「你下一步」；非 Express = 白话摘要表 + 「你下一步」（CORE §用户可见输出）；Full 强制须白话提示完整流程 |
...
| **已知限制** | 合规依赖 Agent 自觉执行硬门禁；PM 脚本 / Git **可选**；冷启动无 project-context 时保守 Standard；L1 须子窗（同会话即可，标注「非独立复核」；多 Step 时 PM **提示**可选新开 Chat，不校验） |
...
改 CORE 后检查 `.mdc` / `.trae/rules` 与各岗 SKILL 指针；并跑 `.ai-gates/scripts/check-rule-invariants.ps1`（承重句 canary）。
Hook 合同：每条 `hooks.json` 须 `timeout`>0 且 `failClosed: false`；读 stdin 的脚本禁止自行无限等（宿主 timeout 杀进程为底线）。
一次性 skill（周报、help、doctor、速查）：调用即产出，禁止切持久模式、写会话 flag、变成 always-on 人格。
...
| 外仓对照（external-compare；常规不启用） | [external-compare.md](./references/external-compare.md)（model-invoked：用户点名「外仓对照」→ 见 [reference-routing.md](./references/reference-routing.md)；**提示 ≠ 启用**；**加载 ≠ 启用**；外仓只作结构参考，禁止复制进本仓；**边界**：≠ [research-task.md](./references/research-task.md) / ≠ [collect-queue.md](./references/collect-queue.md) / ≠ 第七岗） |
...
| 跨项目收集仓（本地 shareable 队列 + GitHub PR） | [collect-queue.md](./references/collect-queue.md)（收集仓已开通：`zhaobolun-code/ai-gates-collect`；未登录默认本地 `.ai-gates/collect-queue.md`；本机探测成功 +「上传」才开 PR；**边界**：≠ [knowledge-gap.md](./references/knowledge-gap.md) / lessons pending / `evolution-candidates.yaml`） |
...
| 模式沉淀（pattern-harvest；错题本同构入表手续；不是岗） | [pattern-harvest.md](./references/pattern-harvest.md)；模板 [pattern-pending.md](./templates/pattern-pending.md)（日常表+模型自动触发表均有行；**禁止静默入表**；pending=`证据/_pattern-pending.md`（**不**塞 `_lesson-pending.md`）；**四格**典故×错题×通用×项目；Skill 自进化=项目格抽象升通用格，收集仓已开通（上传 GitHub → 抽象 → 下发=升级拉真源；合并≠已下发）；**边界**：≠ 第七岗 / ≠ `knowledge-harvest`；Agent 等价落表，先不写 commit 脚本） |
| 电路子窗（circuit-windows；策划契约串/并；不是岗） | [circuit-windows.md](./references/circuit-windows.md)（日常表+模型自动触发表均有行；**加载 ≠ 减审**；**边界**：≠ 第八岗 / ≠ `circuit-job` / ≠ diagnosis-gates §3 并行实现；YAGNI 不写调度脚本） |
| 迷雾地图结构面（fog-map-structure；改当前文档窗须读卡片+一度边；5.0 正式页；不是岗） | [fog-map-structure.md](./references/fog-map-structure.md)（日常表+模型自动触发表均有行；未读邻边不得自称已按 5.0.0 合规；**加载 ≠ 每轮灌整图**；禁止读 HTML / 描述图块坐标；Express 机械改 / 纯代码热修不碰文档窗 / 闲聊不读；出图须 PM 提示用户同意；DIRECTLOG 不是图块、不是方案窗；**边界仍 ≠ bump VERSION** / ≠ USER-GUIDE 灰箱第 7 条） |
| 代码认知地图（code-cognition-map；压缩全图一次装载；不是岗） | [code-cognition-map.md](./references/code-cognition-map.md)（日常表+模型自动触发表均有行；有 project-context 认知卡片节则整节装载；**≠迷雾一张卡**；CodeGraph 不得冒充；压缩禁止摘要 FRAS；改卡优先于只报「仍准」；**禁止安装 AOCI/MCP**；Express 机械改 / 无该节不读） |
| 两隔离槽分歧标注（divergence-annotation；常规不启用） | [divergence-annotation.md](./references/divergence-annotation.md)（model-invoked：高危/止损/跨模块大改/用户点名分歧实验 · 策划或改码前 epistemic 分歧 → 见 [reference-routing.md](./references/reference-routing.md)；**边界**：两隔离槽（不同 id、B 不见 A 链、对照由派发方写）；入队复用 [knowledge-gap.md](./references/knowledge-gap.md)，产物回灌 [isolated-review.md](./references/isolated-review.md) 便宜审、**不**替代隔离审，**不等于**完整碰撞（现行 [collision-review.md](./references/collision-review.md)；5–8x 仍搁置）） |
| 完整思考碰撞（collision-review；异模型三轮薄碰撞；常规不启用） | [collision-review.md](./references/collision-review.md)（model-invoked：用户点名「完整碰撞」**或**（止损触顶/将到 2/3 / 热度爆炸 **且** 确认包选用碰撞）→ 见 [reference-routing.md](./references/reference-routing.md)；**提示 ≠ 启用**；**加载 ≠ 启用**；禁止止损/热度单独启用；**强制异模型轮转**（禁止同模型双链 / resume 同一对）；**三方边界**：≠ [divergence-annotation.md](./references/divergence-annotation.md) 两隔离槽；≠ [isolated-review.md](./references/isolated-review.md) 单向隔离审；启用时复用 isolated-review 派发协议；5–8x 仍非默认） |
| 逆链 / 设计因果闭合（reverse-chain；常规不启用） | [reverse-chain.md](./references/reverse-chain.md)（model-invoked：用户点名「逆链」**或**（高危/止损且方案声明启用）→ 见 [reference-routing.md](./references/reference-routing.md)；**加载 ≠ 启用**；常规不启用；**边界**：≠ [collision-review.md](./references/collision-review.md) / [divergence-annotation.md](./references/divergence-annotation.md) / 独立 ADR 仓库 / 默认六子代理） |
| 逆向总结典故（reverse-allusion；归档压缩提名；无三格跳过） | [reverse-allusion.md](./references/reverse-allusion.md)（model-invoked：结案归档且有改前三格或用户点名「逆向总结典故」→ 见 [reference-routing.md](./references/reference-routing.md)；**加载 ≠ 每个归档必跑**；**边界**：≠ [reverse-chain.md](./references/reverse-chain.md) / [collision-review.md](./references/collision-review.md) / [divergence-annotation.md](./references/divergence-annotation.md)；pending 不写入 design-patterns / shared-language 词条表；晋升仍走 01-B（本机制不实施晋升）） |
...
`.ai-gates/package-release.ps1`（权威入口；`.cursor/package-release.ps1` 若存在则为传送门/旧路径）——把要分发给其他项目的文件打包成 7z：
...
- [ ] 运行 `validate-pipeline.ps1 -Strict`（须含 **hooks policy** / `check-hooks-policy.ps1`：MAINTAINER `deny`（Cursor 2.2+ ask 无效）+ 全 hook `failClosed:false` + `timeout>0` + `sessionStart` 漂移 hook 齐套 + **rule invariants** / `check-rule-invariants.ps1` 承重句 canary + **hooks/、hooks/codex/、scripts 下 *.ps1 全 UTF-8 BOM**；并跑通 `test-hooks.ps1`）
...
- [ ] 传送门验收：`link-platform.ps1`（Windows Junction）幂等重跑 OK；`.cursor/skills|hooks|scripts|rules`、`.cursor/hooks.json`、`.codex`、`.claude`、`.trae/skills` 全部指向 `.ai-gates/`；`git status` 无 `.cursor/`、`.codex/`、`.claude/`、`.trae/` 泄漏；`package-release.ps1` 打包成功且 7z 内含 `link-platform.*` + `codex/` + `claude/`（settings/agents/mcp）接线 + `scripts/fog-map.template.html`、不含 `MAINTAINER.md`、进包 `design-patterns.md` 为空表（无本仓验证行）
- [ ] 机制减负：CORE 翻车索引 ≤15 条且命中驱动（新增条目带近 90 天命中证据；超限最低命中降级回 anti-patterns 完整表）
- [ ] 打包：`package-release.ps1` 纳入 `CHANGELOG.md`、仍排除 `MAINTAINER.md`；进包 `design-patterns.md` 用空表模板覆盖（本机满表不进包）；公开仓/Release 建议挂 CHANGELOG
