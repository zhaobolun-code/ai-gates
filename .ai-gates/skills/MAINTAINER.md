...
| 交付物改动审计 | `preToolUse`（matcher: `Write\|StrReplace\|EditNotebook`） | 记录 `时间戳 \| tool \| session \| path` 一行到 `.ai-gates/hooks-log/write-audit.log`（不提交 Git）；**始终 `allow`，不拦截** | `audit-write.ps1` |
