...
- **Guardrails are not just self-discipline**: after install there is a machine-enforced layer in the wired clients (Cursor, Codex CLI, Claude Code; platform differences—see [Platform](#platform))—editing files out of process is denied on the spot, changing the pipeline itself requires a changelog entry first, and dangerous commands like `push --force` are blocked. When blocked, you are told why and how to proceed.
...
Supports Cursor / Codex / Trae / Claude Code. The machine gates are **not** “every client denies writes”: Cursor, Codex CLI, and Claude Code can block; Codex desktop’s `apply_patch` may not fire; Trae has no machine hooks. The gate scripts are PowerShell: Windows uses its built-in powershell; on macOS / Linux you need PowerShell 7 (`pwsh`) installed—without it, `PM doctor` reports `hooks_not_wired_no_pwsh` and only the rule layer remains. A machine pass only proves “this session said `[PM]` within 120 minutes”—not that this item finished its structured judgment. Platform differences and “what to do when blocked”: see [USER-GUIDE](https://github.com/zhaobolun-code/ai-gates/blob/main/.ai-gates/USER-GUIDE.md).
